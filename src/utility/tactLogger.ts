...
import { TactLogger as TactLoggerType } from '@tact-lang/compiler';
...
const TactLogger: TactLoggerType = {
  log: (message: string) => {
...
const printLog = (message: string, type: 'info' | 'error') => {
  EventEmitter.emit('LOG', {
    text: message,
    type: type,
    timestamp: new Date().toISOString(),
